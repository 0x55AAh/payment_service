...
## Гарантии доставки (Outbox Pattern)

Сервис реализует **Transactional Outbox Pattern**. При создании платежа запись в таблицу `payments` и сообщение в таблицу `outbox` сохраняются в одной транзакции. Это гарантирует, что событие в RabbitMQ будет опубликовано только в случае успешного сохранения платежа в БД.
